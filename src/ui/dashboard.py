...
2. **Sandbox Executor** — runs code in isolated Docker container
...
if "result" in st.session_state:
    ...
    if trace:
        for i, entry in enumerate(trace):
            ...
            icon = {"coder": "✏️", "executor": "⚙️", "critic": "🔍"}.get(node, "➡️")

            with st.expander(f"{icon} Attempt {retry + 1} — {node}", expanded=True):
                st.json(entry)
    else:
